...
// 移动端页面与 API 由同一个 FastAPI 服务提供。旧版本允许保存其他服务地址，
// 会造成登录 Cookie 与 API 不同源，进而出现登录后看不到服务器事件的问题。
localStorage.removeItem("riverPatrolApiBase");
...
function bindEvents() {
  ...
  $("currentServiceUrl").textContent = window.location.origin;
  $("checkConnectionButton").addEventListener("click", () =>
    checkConnection({ notify: true }),
  );
  $("logoutButton").addEventListener("click", async () => {
    await api("/api/auth/logout", { method: "POST" });
    window.location.replace("/login?next=%2Fmobile%2F");
  });
